...
public class ProductPagePOM extends BasePOM {
    ...
    public Product getProductDetails() {
        WebElement titleElem = new WebDriverWait(driver, Duration.ofSeconds(AVERAGE_WAIT_VALUE)).until(ExpectedConditions.presenceOfElementLocated(productName));
        String itemName = titleElem.getText();

        WebElement priceElem = new WebDriverWait(driver, Duration.ofSeconds(AVERAGE_WAIT_VALUE)).until(ExpectedConditions.presenceOfElementLocated(price));
        int itemPrice = Integer.parseInt(priceElem.getDomAttribute(PRICE_AMOUNT_ATTRIBUTE));

        WebElement colorItemElem = new WebDriverWait(driver, Duration.ofSeconds(AVERAGE_WAIT_VALUE)).until(ExpectedConditions.presenceOfElementLocated(color));
        String itemColor = colorItemElem.getText();

        WebElement qtyItemElem = new WebDriverWait(driver, Duration.ofSeconds(AVERAGE_WAIT_VALUE)).until(ExpectedConditions.presenceOfElementLocated(count));
    ...
    public boolean hasSizeSection() {
        try {
            WebElement elem = new WebDriverWait(driver, Duration.ofSeconds(AVERAGE_WAIT_VALUE)).until(ExpectedConditions.presenceOfElementLocated(sizeSection));
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public ConfirmationPopUpPOM addToCart() {
        ...
        elem.click();
        return new ConfirmationPopUpPOM(driver);
    }
    ...
    public String getMaterialValue(String material) {
        String cssSelector = OUTER_MATERIAL + material + "'";
